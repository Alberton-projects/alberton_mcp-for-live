# Alberton — the Remote Script half of Alberton MCP for Live.
#
# Stable loader: executes impl.py from disk on every instantiation, so during
# development the bridge can be edited and re-run by toggling the Control
# Surface (Preferences > Link, Tempo & MIDI) to "None" and back — no Live
# restart. Keep this file frozen; iterate on impl.py. In steady state neither
# file changes: capabilities are added in the MCP server (see docs/CONTRACT.md).
...
def create_instance(c_instance):
    ...
    try:
        with open(impl_path, "r") as fh:
            source = fh.read()
        namespace = {"__file__": impl_path, "__name__": "Alberton_impl"}
        ...
        try:
            ...
            pass
        try:
            c_instance.log_message("Alberton failed to load; see " + _CRASH_LOG)
        except Exception:
            pass
