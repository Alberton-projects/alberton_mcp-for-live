...
- Dumped: 2026-08-02 23:21:41 (pass `settled`), platform `darwin`
- Raw data with full docstrings and the instance-graph evidence: `docs/lom-raw.json`

## Instance coverage

17 of 83 module classes were met as real instances in the set that was open when this was dumped; the rest exist in dir(Live) but nothing here has ever confirmed one. Per class, the header below says which.

Never met: `ATimeableValueVector`, `BeatTime`, `BrowserItemVector`, `CCFeedbackRule`, `CcControlDevice`, `Chain`, `ChainMixerDevice`, `CompressorDevice`, `ControlDescription`, `ControlDescriptionVector`, `ControlSurfaceProxy`, `CuePoint`, `DeviceContainer`, `DeviceIO`, `DriftDevice`, `DrumCellDevice`, `DrumChain`, `DrumPad`, `Envelope`, `EnvelopeEvent`, `EnvelopeEventControlCoefficients`, `EnvelopeEventVector`, `Eq8Device`, `FloatVector`, `HybridReverbDevice`, `IntU64Vector`, `IntVector`, `LimitationError`, `ListenerHandle`, `ListenerVector`, `LomObject`, `LooperDevice`, `MaxDevice`, `MeldDevice`, `MidiNote`, `MidiNoteSpecification`, `MidiNoteVector`, `NoteFeedbackRule`, `ObjectVector`, `PitchBendFeedbackRule`, `PitchClassAndOctave`, `ProgressDialog`, `PythonLicensingBridge`, `RackDevice`, `ReferencePitch`, `RoarDevice`, `RoutingChannelVector`, `RoutingTypeVector`, `Sample`, `ShifterDevice`, `SimplerDevice`, `SmptTime`, `SpectralResonatorDevice`, `StartupDialogServes as an entry point for the user to authorize Live on first launch.`, `StringVector`, `TakeLane`, `Text`, `Timer`, `TuningSystem`, `UnavailableFeatureVector`, `UnlockStatus`, `Variants`, `Vector`, `WarpMarker`, `WarpMarkerVector`, `WavetableDevice`
...
### `Chain`
*bases: DeviceContainer — This class represents a group device chain in Live. — never met as an instance on this machine*
...
### `ChainMixerDevice`
*bases: LomObject — This class represents a Chain's Mixer Device in Live, which gives you — never met as an instance on this machine*
...
### `WarpMarker`
*This class represents a WarpMarker type. — never met as an instance on this machine*
...
### `CompressorDevice`
*bases: Device — This class represents a Compressor device. — never met as an instance on this machine*
...
### `DrumChain`
*bases: Chain — This class represents a drum group device chain in Live. — never met as an instance on this machine*
...
### `DrumPad`
*bases: LomObject — This class represents a drum group device pad in Live. — never met as an instance on this machine*
...
### `Envelope`
*bases: LomObject — This class represents an automation or modulation envelope in Live. — never met as an instance on this machine*
...
### `Eq8Device`
*bases: Device — This class represents an Eq8 device. — never met as an instance on this machine*
...
### `MaxDevice`
*bases: Device — This class represents a Max for Live device. — never met as an instance on this machine*
...
### `RackDevice`
*bases: Device — This class represents a Rack device. — never met as an instance on this machine*
...
### `Sample`
*bases: LomObject — This class represents a sample file loaded into a Simpler instance. — never met as an instance on this machine*
...
### `SimplerDevice`
*bases: Device — This class represents a Simpler device. — never met as an instance on this machine*
...
### `TakeLane`
*bases: LomObject — This class represents a take lane in Live. — never met as an instance on this machine*
